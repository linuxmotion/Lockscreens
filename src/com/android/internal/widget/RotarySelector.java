...
public class RotarySelector extends View {
    ...
    // This value is actault in the calss View 
    // but is hidden from the sdk
    private int mRight;
    ...
    public RotarySelector(Context context, AttributeSet attrs) {
        super(context, attrs);
        if (DBG) log("Creating the view from Attrs and Context");

        mContext = context;
        //TypedArray a =
            //context.obtainStyledAttributes(attrs, R.styleable.RotarySelector);
        mOrientation = 0;//a.getInt(R.styleable.RotarySelector_orientation, HORIZONTAL);
       // a.recycle();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isHoriz()) {
          ...
          /// Set the amount of pixels from the left side of the view to the right
            mRight = length;
        } else {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if (DBG) log("Beginin to draw the view");
        ...
        if (DBG) log("Finished redrawing the view");
    }
    ...
    private int getYOnArc(int backgroundWidth, int innerRadius, int outerRadius, int x) {
    	if (DBG) log("Retriving the y on the arc");
        // the hypotenuse
    ...
    public boolean onTouchEvent(MotionEvent event) {
    	if (DBG) log("Received a touch input");
        if (mAnimating) {
            return true;
        }
    ...
    private void drawCentered(Bitmap d, Canvas c, int x, int y) {
    	if (DBG) log("Drawing the bitmap centered");
        int w = d.getWidth();
        int h = d.getHeight();
    ...
    public void setRevamped(boolean newState){
    	if (DBG) log("Setting rotary revamped");
        if(newState){
            if(mCustomAppDimple)
    ...
    public void setLenseSquare(boolean newState){
    	if (DBG) log("Setting the lense style");
        mLenseMode=false;
        if(newState){
            mLenseMode=true;
